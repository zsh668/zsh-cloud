...
public class UserConverter {
     ...
     * @return
     */
    public static User toUser(SysUserDO sysUserDO, List<Role> roles, List<UserGroup> groups) {
        if (sysUserDO == null) {
            return null;
        }
        
        List<RoleId> roleIds = roles.stream().map(Role::getRoleId).collect(Collectors.toList());
        List<RoleName> roleNames = roles.stream().map(Role::getRoleName).collect(Collectors.toList());
        
        List<UserGroupName> groupNames = groups.stream().map(UserGroup::getUserGroupName).collect(Collectors.toList());
        ...
        return new User(new UserId(sysUserDO.getId()), new Account(sysUserDO.getAccount()),
                ...
                sysUserDO.getPasswordExpireTime(), sysUserDO.getLastLoginTime(), sysUserDO.getWorkDescribe(), orgId,
                stationId, tenantId, sysUserDO.getCreatedTime(), sysUserDO.getUpdatedTime(), roleIds, roleNames,
                groupNames);
    }
