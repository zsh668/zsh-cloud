...
import com.zsh.cloud.common.core.domain.Sort;
...
public class MyBatisUtils {
    ...
    public static <T> Page<T> buildPage(PageQuery pageQuery) {
        return buildPage(pageQuery, null);
    }
    ...
    public static <T> Page<T> buildPage(PageQuery pageQuery, Collection<Sort> sortFields) {
        ...
        if (!CollectionUtil.isEmpty(sortFields)) {
            page.addOrder(sortFields.stream()
                    .map(sortingField -> Sort.ASC.equals(sortingField.getOrder()) ? OrderItem.asc(
                            sortingField.getField()) : OrderItem.desc(sortingField.getField()))
    ...
    /**
     * 获得 Table 对应的表名. 兼容 MySQL 转义表名 `t_xxx`
     *
     * @param table 表
     * @return 去除转移字符后的表名
     */
